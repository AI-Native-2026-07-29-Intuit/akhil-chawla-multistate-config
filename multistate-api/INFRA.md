...
| **D4 T1** Cost stack + tags | `multistate-cost-dev` `CREATE_COMPLETE`; SNS TopicPolicy for budgets + cloudwatch; Budget + alarm in template; NAT + RDS tags in YAML; `describe-stacks` / `describe-budget` / `describe-alarms` on Floci where supported | **Cost Explorer** saved report with NAT line item; Budget notification delivery; `AWS/Billing` metric data (alarm stays INSUFFICIENT_DATA); cost-allocation tag activation in Billing console |
| **Hygiene** | Branch `w6d4-implementation`; annotated YAML; this document | None |
